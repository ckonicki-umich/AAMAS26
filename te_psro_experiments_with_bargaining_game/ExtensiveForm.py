...
class ExtensiveForm:
	...
	def get_infoset_given_node(self, node):
		...
		def find_infoset(node):
			for x in self.infosets[node.player_id - 1]:
				if x.infoset_id == node.infoset_id:
					yield x

		matching_infosets = list(find_infoset(node))
	...
	def get_next_infoset_id(self, path, i, cur_player_id):
		'''
		'''
		next_player_id = (i + 1) % 2 + 1
		next_infoset_id = None
		offer_reveal = self.check_outside_offer_reveal(path[:(i + 1)], cur_player_id)
		if next_player_id == 1:

			empir_history = tuple(path)[:1] + tuple(path)[2:(i + 1)]
			if offer_reveal:
			...
			empir_history = tuple(path)[1:(i + 1)]
			if offer_reveal:
				empir_history = tuple(path)[:(i + 1)]
	...
	def update_game_with_simulation_output(self, observations, payoffs):
		...
		for tup_path in observations:
			...
			for i in range(len(path)):
				...
				if i in [0, 1]:
					...
					children_matching = list(gen_matching_children(cur_node.children, a))
					if len(children_matching) == 0:
						...
						if i == 1:
							next_player_id = 1
							next_infoset_id = (1, tuple(path)[:1])
							...
							if matching_infosets == []:
								next_infoset = Infoset(next_infoset_id, [next_node], [], 2, None)
							...
							next_player_id = 0
							next_infoset_id = (0, tuple(path)[:(i + 1)])
							next_node = Node(next_player_id, next_infoset_id, cur_node.history + [a], [], 2)
							cur_node.add_children([next_node])

					else:
						path_to_match = (path[0],)
						def gen_next_nodes(children, a, path_to_match):
							for x in children:
								if a == x.history[-1] and x.infoset_id[1] == path_to_match:
									yield x

						next_nodes = list(gen_next_nodes(cur_node.children, a, path_to_match))
						next_node = next_nodes[0]

						# cur_node is a chance node outputting o1 or o2
						cur_dist = self.chance_map[cur_node].copy()
						cur_dist[a] = cur_dist.get(a, 0.0) + observations.get(tup_path)
						self.chance_map[cur_node] = cur_dist
					...
					if len(children_matching) == 0:
						...
						next_infoset_id = self.get_next_infoset_id(path, i, cur_player_id)
		...
		for t in self.terminal_nodes:
			payoffs_t = payoffs[tuple(t.history)]
			t.utility = np.sum(payoffs_t, axis=0) / len(payoffs_t)

		for i in range(2):
			for infoset in self.infosets[i]:
				for n in infoset.node_list:
					assert n.action_space == infoset.action_space
					for a in n.action_space:
						new_history = n.history + [a]
						c_list = [c.history for c in n.children if c.history == new_history]
						assert len(c_list) <= 1

					assert len(n.children) <= len(n.action_space)
	...
	def check_outside_offer_reveal(self, history, player_num):
		'''
