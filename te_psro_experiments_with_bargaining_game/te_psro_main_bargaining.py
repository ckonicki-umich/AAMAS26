...
def main(game_param_map, T, trial_index, br_mss, eval_strat):
	...
	while len(empirical_game_size_over_time) < NUM_PSRO_ITER and (regret_over_time[-1] > REGRET_THRESH):
		...
		np.savez_compressed(file_prefix + str(NUM_EMPIR_BR) + '_' + file_ID + '_' + str(trial_index), regret_over_time, max_local_regret_over_time, empirical_game_size_over_time, 
			ne_over_time, pbe_over_time)
...
game_params = retrieve_game(file_ID_index)
