...
def recursive_empirical_pay_infoset_helper(br_meta_strat, true_history, empir_history, input_reach_prob, infoset_reach_prob, infoset_id, game_param_map, 
	...
	# print("input infoset ", infoset_id)
	# print(round_num)
	...
	if num_p2_actions == (num_rounds - 1):
		...
		# 7/20: Rewrote to acquire empirical_infoset_id given empirical history, not the true history
		#empir_infoset_id = get_empirical_infoset_id_given_history(true_history, game_params, POLICY_SPACE1, POLICY_SPACE2)
		empir_infoset_id = get_empirical_infoset_id_given_empir_history(empir_history, player_num)
		#print("empir_infoset_id ", empir_infoset_id)
		input_empir_actions = [a for a in infoset_id[1]]
		# cur_empir_actions = [a for a in empir_infoset_id[1]]
		...
		if len(cur_empir_actions) < len(input_empir_actions):
			...
			#print("choose BR player's actions so they lead to the given infoset")
			infoset_freq = 0.0
			...
			if infoset_strat is not None:
				...
				#(A, pi_0, pi_2, B, pi_0, pi_1, pi_0)
				# (A, pi_0, pi_2, B, pi_0, pi_1)
