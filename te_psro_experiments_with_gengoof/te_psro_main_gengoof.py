...
def main(game_param_map, T, trial_index, br_mss, eval_strat):
	'''
	...
	@arg (int) T: Number of iterations for a single run of CFR or PBE-CFR (whether solving a game for NE or PBE)
	@arg (str) br_mss: identification for which solution type we will use as the MSS to find best responses
		for -- at present, either "ne" or "pbe"
	...
	Runs a single play of TE-PSRO on large abstract game, expanding strategy space, simulating
	...
	need_PBE = (eval_strat == "PBE") or (br_mss == "PBE")
	...
	while len(empirical_game_size_over_time) < NUM_PSRO_ITER and (regret_over_time[-1] > REGRET_THRESH):
		...
		if need_PBE:
			...
			if eval_strat == "PBE":
				regret_meta_strat = pbe_ms[0]
			if br_mss == "PBE":
				br_meta_strat = pbe_ms[0]
			pbe_over_time.append(pbe_ms)

			print("computing max_local_regret")
			max_local_regret = empirical_game.verify_sequent_rat(pbe_ms)
			print("max_local_regret ", max_local_regret)
			max_local_regret_over_time.append(max_local_regret)

		print("regret_meta_strat")
		for x in regret_meta_strat:
			print(x, regret_meta_strat[x])
		print(regret_meta_strat)
		...
		np.savez_compressed(prefix, regret_over_time, max_local_regret_over_time, empirical_game_size_over_time, 
			np.array(ne_over_time, dtype=object), np.array(pbe_over_time, dtype=object))

	del mgame['game']
	mgame.vacuum()


# total number of files = 15 * (NUM_ROUNDS - 1)
'''
MAIN CODE
'''
...
main(game_param_map, T, trial_index, br_mss=args.mss.upper(), eval_strat=args.eval.upper())
